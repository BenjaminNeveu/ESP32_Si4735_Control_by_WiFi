...
void setup() {
  ...
  // Cette suite d'instruction permet de configurer le recepteur en fm
  ...
  // Cette permet a la page web d'afficher la frequence
  server.on("/getFrequency", HTTP_GET, [](AsyncWebServerRequest * request) {
    // on recupere la valeur de la frequence que l'on place dans une variable
    previousFrequency = currentFrequency = rx.getFrequency();
    // la fréquence est ecris par défault KHz on la modifie pour quel soit MHz
  ...
  server.on("/getPilot", HTTP_GET, [](AsyncWebServerRequest * request) {
    rx.getCurrentReceivedSignalQuality();
    String pilot = ((rx.getCurrentPilot()) ? "STEREO" : "MONO");
    request->send(200, "text/plain", pilot);
  });
